<template>
<div class="diplomes">
  <h3 class="subheading grey--text">Diplomes</h3>
 ...
 <v-spacer></v-spacer>
      ...
      <v-card>
          <v-card-title>
              <span class="headline">Ajouter un Diplome </span>
          </v-card-title>
          <v-card-text>
              <v-container>
                  <v-row>
          ...
          <v-card-actions>
              ...
              <v-btn color="blue darken-1" text @click="savediplomes">Save</v-btn>
          </v-card-actions>
      </v-card>
  </v-dialog>
  ...
  <!--fin modal d'ajout-->
                        ...
                        <v-dialog v-model="dialogEdit" persistent max-width="600px">
                            <v-card>
                                ...
                                <v-card-actions>
                                    ...
                                    <v-btn color="blue darken-1" text @click="updatediplomes">Update</v-btn>
                                </v-card-actions>
                            </v-card>
                        </v-dialog>
    ...
    <v-data-table :headers="headers" :items="diplomes" :search="search"  class="elevation-1">
      <template v-slot:[`item.actions`]="{ item }">
      <v-icon color="#3b3e66" class="mr-2" @click="getEdit(item)"> mdi-pencil </v-icon>
      <v-icon color="#3b3e66" @click="deletediplomes(item)">mdi-delete</v-icon>
      </template>
   </v-data-table>
...
<script>
 ...
 methods: {    
       ...
       getdiplomes : function () {
            axios
            .get('http://localhost:8000/api/diplome')
            .then((res) => {
  ...
  // Create New produit
        savediplomes: function () {
            axios.post('http://localhost:8000/api/diplome/', {
 ...
 // Get Edit and Show data to Modal
        ...
        // Update produit
        updatediplomes: function () {
            axios.put(`http://localhost:8000/api/diplome/` + this.id, {
                   libelle: this.libelle,
                    niveau: this.niveau,
                })
                .then(res => {
...
//Get Delete and Show Confirm Modal
 ...
 this.id = item.id;
   ...
   // Delete investigations
        deletediplomes: function (item) {
            if(confirm("vous voulez vraiment supprimer ce type de contrat ?")){
                axios.delete(`http://localhost:8000/api/diplome/`+item.id)
                .then(res => {
                    // handle success
                    this.getdiplomes();
