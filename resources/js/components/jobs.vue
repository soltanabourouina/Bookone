<template>
<div class="jobs">
  <h3 class="subheading grey--text">jobs</h3>
 ...
 <v-spacer></v-spacer>
    ...
    <v-dialog v-model="dialogAdd" persistent max-width="600px">
      ...
      <v-card>
          <v-card-title>
              <span class="headline">Ajouter un Job</span>
          </v-card-title>
          <v-card-text>
              <v-container>
                  <v-row>
                     ...
                     <v-col cols="12">
                          <v-text-field label="code job*" v-model="code" required>
                          </v-text-field>
                      </v-col>
                          <v-col
                          class="d-flex"
          ...
          <v-card-actions>
              ...
              <v-btn color="blue darken-1" text @click="savejobs">Save</v-btn>
          </v-card-actions>
      </v-card>
  </v-dialog>
  ...
  <!--fin modal d'ajout-->
      <!-- Modal Update Product -->
                        ...
                        <v-dialog v-model="dialogEdit" persistent max-width="600px">
                            <v-card>
                                <v-card-title>
                                    <span class="headline">Modifier un Job</span>
                                </v-card-title>
                                <v-card-text>
                                    <v-container>
                                        <v-row>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="blue darken-1" text @click="updatejobs">Update</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
    ...
    <v-data-table :headers="headers" :items="jobs" :search="search"  class="elevation-1">
      <template v-slot: item.actions="{ item }">
        ...
        <v-icon small @click="deletejobs(item)" >mdi-delete</v-icon>
      </template>
    </v-data-table>
...
<script>
 ...
 methods: {    
  ...
  // Create New produit
        savejobs: function () {
            axios.post('http://localhost:8000/api/job/', {
 ...
 // Get Edit and Show data to Modal
        ...
        // Update produit
        updatejobs: function () {
            axios.put(`http://localhost:8000/api/job/` + this.id, {
                   libelle: this.libelle,
                   code: this.code,
                    metier: this.metier,
                })
...
//Get Delete and Show Confirm Modal
 ...
 this.id = item.id;
   ...
   // Delete investigations
        deletejobs: function (item) {
            if(confirm("vous voulez vraiment supprimer ce job ?")){
                axios.delete(`http://localhost:8000/api/job/`+item.id)
                .then(res => {
                    // handle success
                    this.getjobs();
